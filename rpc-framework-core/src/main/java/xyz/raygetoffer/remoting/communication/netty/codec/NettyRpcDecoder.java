...
//            byteBuf.release();
    ...
    private Object decodeFrame(ByteBuf in) {
        ...
        if (dataLength > 0) {
            ...
            in.readBytes(bytes);
            // 解压数据
            ICompress compress = ExtensionLoader.getExtensionLoader(ICompress.class).getExtension(CompressTypeEnum.getName(compressType));
            bytes = compress.decompress(bytes);
            // 反序列化数据
            ISerializer serializer = ExtensionLoader.getExtensionLoader(ISerializer.class).getExtension(SerializationTypeEnum.getName(codecType));
